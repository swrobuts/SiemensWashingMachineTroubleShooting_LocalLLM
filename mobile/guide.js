(function (root) {
    ...
    function render(data) {
        ...
        return '<p class="eyebrow">WASCHMASCHINEN-ASSISTENT · ANLEITUNG</p><h1>' + escape(data.question || 'Hinweise aus dem Handbuch') + '</h1>' +
            ...
            '<p id="progress"></p><ol class="steps">' + data.steps.map(text => '<li class="step' + (/Warnung|Gefahr|abkühlen|Netzstecker/i.test(text) ? ' warning' : '') + '">' + icon(text) + '<div><h2>Schritt</h2><p>' + escape(text) + '</p><label><input type="checkbox"> Erledigt</label></div></li>').join('') + '</ol>' +
            ...
            '<p class="note">Diese Ansicht übernimmt die erzeugte Antwort unverändert. Symbole dienen der Orientierung und zeigen keine gerätespezifischen Bauteile. Prüfen Sie die Bedienungsanleitung Ihres Geräts und beachten Sie deren Sicherheits- und Kundendiensthinweise.</p>';
    }
    function speechText(data) {
        return [data.summary, data.intro, ...data.steps.map((text,i) => `Schritt ${i+1}. ${text}`)].filter(Boolean).join('\n');
    }
